---
title: "CONGRESO"
subtitle: "Informe Institucional de Resultados Caracterización Cuestionarios Perfeccionismo y Tenacidad "
author: "Psic. Ma. Fernanda Serrano García"
date: 2024-05-08
output: hugodown::md_document
---
```{r setup, include=FALSE}
#output: hugodown::md_document
knitr::opts_chunk$set(echo = FALSE, warning = FALSE, message = FALSE)
## load processing script
source('https://github.com/Mafer-sg/altas-capacidades/raw/main/mxco.R')
```

# USAGE DOCUMENTATION
## TABLES EXAMPLES
To show a level-percentage table by school:

The elements of the list

- First element:

 [1] "andes"         "bicentenario"  "cepac"         "coltec"

 [5] "diosa"         "esperanza"     "pidahi"        "primcongreso"

 [9] "secaugusto"    "villavicencio"

- Second element:

[1] "fG_level"           "tenacidad_level"         "esfuerzo_level"

[4] "preocupacion_level"

### Table Augusto Intelligence

- Use "caption" field to modify the title.

```{r, echo=FALSE}
knitr::kable(list_extracted_levels_datasets[['primcongreso']][['fG_level']], caption = 'Nivel de Inteligencia', digits = 1)
```
### Table Augusto Tenacidad

- Use "caption" field to modify the title.

```{r, echo=FALSE}
knitr::kable(list_extracted_levels_datasets[['primcongreso']][['tenacidad_level']], caption = 'Nivel de Tenacidad', digits = 1)
```
### Table Augusto Esfuerzo

- Use "caption" field to modify the title.

```{r, echo=FALSE}
knitr::kable(list_extracted_levels_datasets[['primcongreso']][['esfuerzo_level']], caption = 'Nivel de Esfuerzo', digits = 1)
```
### Table Augusto Preocupacion

- Use "caption" field to modify the title.

```{r, echo=FALSE}
knitr::kable(list_extracted_levels_datasets[['primcongreso']][['preocupacion_level']], caption = 'Nivel de Preocupacion', digits = 1)
```

## Showing a mean-sd table by school
Note that there is only one mean-sd table by school

### Mean and Standard deviation Table for Augusto

- Use "caption" field to modify the title.

```{r, echo=FALSE}
knitr::kable(list_by_school_mean_sd_tables[['primcongreso']], caption = 'primcongreso', digits = 1)
```

## GRAPHICS EXAMPLES
Showing a level-percentage graphic:

Use the same elements as the level-percent table, but set 'graph' in the last element.

### Percent-Levels Augusto Intelligence

```{r, echo=FALSE}
list_by_school_levels_percents_tables[['primcongreso']][['fG_level']][['graph']]
```

### Percent-Levels Augusto Tenacity

```{r, echo=FALSE}
list_by_school_levels_percents_tables[['primcongreso']][['tenacidad_level']][['graph']]
```

### Percent-Levels Augusto Effort

```{r, echo=FALSE}
list_by_school_levels_percents_tables[['primcongreso']][['esfuerzo_level']][['graph']]
```

### Percent-Levels Augusto Preoccupation

```{r, echo=FALSE}
list_by_school_levels_percents_tables[['primcongreso']][['preocupacion_level']][['graph']]
```

## MEAN AND SD GRAPHS
Showing a main-sd point inverval plot:

- First element is school name same as examples above.

- Second elemnt:

 "fG"     "Tenaci" "Esfuer" "Preocu"


### Example Augusto Intelligence

```{r, echo=FALSE}
list_by_school_mean_graphs[['primcongreso']][['fG']]
```

### Example Augusto Tenacidad

```{r, echo=FALSE}
list_by_school_mean_graphs[['primcongreso']][['Tenacidad']]
```

### Example Augusto Esfuerzo

```{r, echo=FALSE}
list_by_school_mean_graphs[['primcongreso']][['Esfuerzo']]
```

### Example Augusto Preocupacion

```{r, echo=FALSE}
list_by_school_mean_graphs[['primcongreso']][['Preocupacion']]
```

# Mafer's REPORT
El presente informe es una síntesis de los hallazgos producto del análisis de las pruebas de perfeccionismo y tenacidad, aplicadas a los estudiantes de su institución de los grados 1° a 3°, quienes participaron en éste ejercicio, mediante el cual se busca brindarles información valiosa de los estudiantes, que sirvan de complemento a otras medidas y datos disponibles, favoreciendo el diseño e implementación de estrategias de orientación psicoeducativa. Es importante aclarar que se trata de resultados preliminares, dado que las pruebas se encuentran en proceso de validación. 
A continuación se presenta una breve información sobre las variables y las pruebas, con referencia a la población de la propia institución y con el fin de ayudarles en la interpretación de los datos:
IMPORTANTE: Estos resultados deben ser usados únicamente en pro de los estudiantes y nunca con fines de discriminación ni diagnóstico. 
