...
pmat <- round(corr.test(rows_dataset, columns_dataset)$p, 3)
	...
	for(eachcol in cols) {
		...
		if(length(pairs_list) > 0)
		{
			...
			library('gridExtra')
			graph_grid <- arrangeGrob(grobs=scatters_list, ncol=3)
...
scaterp_list[[length(scaterp_list)+1]] <- 
    ggcorrplot(t(cor_mat), ggtheme = ggplot2::theme_dark, lab=TRUE, p.mat=t(cor_test_mat), insig='blank')
return(scaterp_list)
}
```
